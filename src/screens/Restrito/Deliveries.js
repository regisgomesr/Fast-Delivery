  ...
  renderDelivery = (delivery) => {
    return (
      <Table.Row key={delivery.id}>
        <Table.Cell>{delivery.name_client}</Table.Cell>
        <Table.Cell>{delivery.starting_point}</Table.Cell>
        <Table.Cell>{delivery.destination_point}</Table.Cell>
        ...
        <Table.Cell>
          <Button
            ...
            as={Link}
            to={`/restrito/deliveries/${delivery.id}/route`}
          >
            Rota
          </Button>
        </Table.Cell>
  ...
  render() {
    return (
      <div>
        ...
        {!this.props.deliveries.isLoading &&
          this.props.deliveries.data.length === 0 && (
